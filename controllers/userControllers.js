...
exports.loginUser = async (req, res) => {
  ...
  if (!user) {
    return res.json({ message: " no user Invalid email or password" });
  }

  const isPasswordValid = await bcrypt.compare(password, user.password);

  if (!isPasswordValid) {
    return res.json({ message: " pass not Invalid email or password" });
  }
  ...
  res.json({ token: token, user: user, message: "login" });
};
